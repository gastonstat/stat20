...
::: {.fragment}
Also known as the __Gaussian__ distribution.
:::
...
::: {.fragment}
$$
X \sim N(\mu, \sigma) \quad \text{or} \quad X \sim N(\mu, \sigma^2)
$$
where $\mu$ is the mean, and $\sigma$ is the SD ($\sigma^2$ is the Var).
:::
...
```{r eval = FALSE}
# 1 - F(6)
pnorm(6, mean = 5, sd = 2, lower.tail = FALSE)
```
:::
